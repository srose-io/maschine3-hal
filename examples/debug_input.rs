use mk3_hal::{MaschineMK3Hid, MK3Error};
    ...
    loop {
        match device.read_input_raw() {
            Ok(data) if !data.is_empty() => {
                match data[0] {
                    0x01 => {
                        println!("📊 Button/Knob packet ({}B): {:?}", data.len(), &data[..std::cmp::min(12, data.len())]);
                        if data.len() >= 42 {
                            // Show some key button bytes
                            println!("   Buttons: byte1={:08b} byte2={:08b} byte6={:08b}", data[1], data[2], data[6]);
                            if data[6] & 0x20 != 0 { println!("   ▶️  PLAY pressed!"); }
                            if data[2] & 0x01 != 0 { println!("   🅰️  Group A pressed!"); }
                        }
                    }
                    0x02 => {
                        println!("🥁 Pad packet ({}B): {:?}", data.len(), &data[..std::cmp::min(20, data.len())]);
                    ...
                    other => {
                        println!("❓ Unknown packet type 0x{:02X} ({}B): {:?}", other, data.len(), &data[..std::cmp::min(8, data.len())]);
                    }
                }
            }
            Ok(_) => {
