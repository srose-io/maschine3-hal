...
fn print_button_state(input: &InputState, changes: &[String], frame: u32) {
    ...
    // Encoder
    if input.buttons.encoder_push
        || input.buttons.encoder_up
        || input.buttons.encoder_down
        || input.buttons.encoder_left
        || input.buttons.encoder_right
    {
        let mut encoder_actions = Vec::new();
        if input.buttons.encoder_push {
            encoder_actions.push("Push");
        }
        if input.buttons.encoder_up {
            encoder_actions.push("Up");
        }
        if input.buttons.encoder_down {
            encoder_actions.push("Down");
        }
        if input.buttons.encoder_left {
            encoder_actions.push("Left");
        }
        if input.buttons.encoder_right {
            encoder_actions.push("Right");
        }
        println!(
            "  🕹️  Encoder: {} (pos: {})",
            encoder_actions.join("+"),
            input.knobs.main_encoder
        );
    }
    ...
    if input.touch_strip.finger_1.data_a > 0 || input.touch_strip.finger_2.data_a > 0 {
        println!(
            "  👆 Touch Strip: F1({},{},{},{}) F2({},{},{},{})",
            input.touch_strip.finger_1.data_a,
            input.touch_strip.finger_1.data_b,
            input.touch_strip.finger_1.data_c,
            input.touch_strip.finger_1.data_d,
            input.touch_strip.finger_2.data_a,
            input.touch_strip.finger_2.data_b,
            input.touch_strip.finger_2.data_c,
            input.touch_strip.finger_2.data_d
        );
    }
    ...
    if input.buttons.pedal_connected || input.buttons.microphone_connected {
        ...
        if input.buttons.pedal_connected {
            status.push("Pedal");
        }
        if input.buttons.microphone_connected {
            status.push("Microphone");
        }
...
fn check_button_changes(current: &InputState, last: &InputState, changes: &mut Vec<String>) {
    ...
    if current.buttons.play != last.buttons.play {
        changes.push(format!(
            "Play: {}",
            if current.buttons.play { "ON" } else { "OFF" }
        ));
    }
    if current.buttons.rec != last.buttons.rec {
        changes.push(format!(
            "Rec: {}",
            if current.buttons.rec { "ON" } else { "OFF" }
        ));
    }
    if current.buttons.stop != last.buttons.stop {
        changes.push(format!(
            "Stop: {}",
            if current.buttons.stop { "ON" } else { "OFF" }
        ));
    }
...
fn check_knob_changes(current: &InputState, last: &InputState, changes: &mut Vec<String>) {
    ...
    if current.knobs.main_encoder != last.knobs.main_encoder {
        changes.push(format!(
            "Main Encoder: {} -> {}",
            last.knobs.main_encoder, current.knobs.main_encoder
        ));
    }
}
